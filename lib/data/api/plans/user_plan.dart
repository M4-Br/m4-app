...
Future<UserPlanModel> getUserPlans() async {
  ...
  try {
    if (response.statusCode >= 200 && response.statusCode <= 299) {
      ...
      final plans =
          jsonResponse.map((item) => UserPlanModel.fromJson(item)).toList();
      // await SharedPreferencesFunctions.saveString(key: 'userPlanUUID', value: plans[0].items[0].id); //SALVAR UUID
      return plans.first;
    } else {
